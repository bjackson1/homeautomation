...
class control:
  def __init__(self, config):
    ...
    self.reversed = True if ('reversed' in config and str(config['reversed']).lower() == 'true') else False
    self.statuspin = int(config['statuspin']) if 'statuspin' in config else 0
