...
class configunittests(unittest.TestCase):
    ...
    def test_reload_WhenCalledWithMockedFile_ReturnsInputDataAsStructure(self):
        with mock.patch('builtins.open', new_callable=mock_open, read_data='---\nservices:\n  redis:\n    host: 127.0.0.1\n    port: 6379\nconfig:\n  test: 123\ntempsensors:\n  test1:\n    host: 127.0.0.1\n    hardwareid: 28-000000000001\n    name: Test 1\ncontrols:\n  landinglight:\n    host: 192.168.1.12\n    controlpin: 21\n    name: Landing Light') as mocked_file, mock.patch('storage.redisclient.getasstring', return_value=19.1) as mocked_redis:
            ctl = controller()
            ctl.reload()
            self.assertEqual(19.1, ctl.sensors['test1'].get())
